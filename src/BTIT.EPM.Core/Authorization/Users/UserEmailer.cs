...
using BTIT.EPM.Localization;
using BTIT.EPM.MultiTenancy;
using System.Net.Mail;
using System.Web;
using Abp.Runtime.Security;
using BTIT.EPM.Net.Emailing;
...
namespace BTIT.EPM.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : EPMServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        /// </param>
        [UnitOfWork]
        public virtual async Task SendEmailViewAndSignDocumentAsync(int tenantId, string firstName, string lastName, string email,
            long recipientId, string documentTitle, string link, long documentRequestId, bool isSignRequired
            , AuditTrailType DocumentStatus, string recipientCode)
        {
            if (recipientCode.IsNullOrEmpty())
            {
                throw new Exception("Document Code should be set in order to send document link.");
            }
            link = link.Replace("{recipientId}", recipientId.ToString());
            link = link.Replace("{documentRequestId}", documentRequestId.ToString());
            link = link.Replace("{recipientCode}", Uri.EscapeDataString(recipientCode));
            ...
            var emailTemplate = GetTitleAndSubTitle(tenantId, isSignRequired ? L("EmailSignDocument_Title") : DocumentStatus == AuditTrailType.Sent ? L("EmailReviewDocument_Title") : DocumentStatus == AuditTrailType.Completed ? L("EmailCompletedDocument_Title") : string.Empty
                , isSignRequired ? L("EmailSignDocument_SubTitle") : DocumentStatus == AuditTrailType.Sent ? L("EmailReviewDocument_SubTitle") : DocumentStatus == AuditTrailType.Completed ? L("EmailCompletedDocument_SubTitle") : string.Empty);
            
            var mailMessage = new StringBuilder();

            mailMessage.AppendLine("<b>" + L("NameSurname") + "</b>: " + firstName + " " + lastName + "<br />");
            ...
            mailMessage.AppendLine("<b>" + L("DocumentTitle") + "</b>: " + documentTitle + "<br />");

            //if (!plainPin.IsNullOrEmpty())
            //{
            //    mailMessage.AppendLine("<b>" + L("PlainPin") + "</b>: " + plainPin + "<br />");
            //}

            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine(isSignRequired ? L("EmailSignDocument_ClickTheLinkBelowToSignTheDocument") : L("EmailSignDocument_ClickTheLinkBelowToReviewTheDocument") + "<br /><br />");
            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine("<a style=\"" + _emailButtonStyle + "\" bg-color=\"" + _emailButtonColor + "\" href=\"" + link + "\">" + L("Review") + "</a>");
            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine("<span style=\"font-size: 9pt;\">" + L("EmailMessage_CopyTheLinkBelowToYourBrowser") + "</span><br />");
            ...
            await ReplaceBodyAndSend(email, isSignRequired ? L("EmailSignDocument_Subject") : L("EmailReviewDocument_Subject"), emailTemplate, mailMessage);
        }
